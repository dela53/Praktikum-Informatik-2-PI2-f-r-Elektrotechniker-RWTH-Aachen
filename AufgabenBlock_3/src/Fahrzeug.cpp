...
void Fahrzeug::vKopf()
{
	std::cout << std::resetiosflags(std::ios::adjustfield)   // alles zurücksetzen
	   ...
	   << std::endl
	   << std::setfill('-')
	   << std::setw(185) << "-" << std::endl << std::setfill(' ');
}
...
void Fahrzeug::vNeueStrecke(Weg& weg)
{
	...
	// Beim Wechsel auf einen neuen Weg Abschnitt auf 0 setzen
...
void Fahrzeug::vNeueStrecke(Weg& weg, double dStartzeitpunkt)
{
	...
	// Parkende Fahrzeuge stehen am Anfang des Weges
	p_dAbschnittStrecke = 0.0;
}

void Fahrzeug::vEinlesen(std::istream& c)
{
	std::string sName;
	...
	if (!c) throw std::runtime_error("Fahrzeug::vEinlesen: Name wurde schon gesetzt.");
}
...
// Was ist friend? Erlaubt es ein private Member auszugeben. --> unschön
